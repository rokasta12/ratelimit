 ...
 */
function createNuxtStorageStore(
  storageKey: string,
  ...
  const store: RateLimitStore = {
    ...
    async increment(key: string): Promise<StoreResult> {
      ...
      // Get existing entry
      const existing = await storage.getItem<{ count: number; reset: number }>(
        fullKey,
      );
    ...
    async get(key: string): Promise<StoreResult | undefined> {
      const fullKey = getKey(key);
      const entry = await storage.getItem<{ count: number; reset: number }>(
        fullKey,
      );
